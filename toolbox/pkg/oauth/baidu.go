...
func GetBaiduOauthToken(ctx context.Context, redisClient *redis.Client, config config.Config) string {
	cmd := redisClient.Get(ctx, "baiduAccessTokenKey")
	...
	url := fmt.Sprintf("%soauth/2.0/token?client_id=%s&client_secret=%s&grant_type=client_credentials",
		config.BaiduOauth.OauthUrl, config.BaiduOauth.AppKey, config.BaiduOauth.AppSecret)
	...
	// 将数据存入redis
	cmd2 := redisClient.Set(ctx, "baiduAccessTokenKey", data.AccessToken, time.Duration(int64(time.Second)*data.ExpiresIn))
...
func AnalysisPictureText(ctx context.Context, redisClient *redis.Client, config config.Config, requestType string, file string, fileType int) (out *IdentifyPictureRes, err error) {
	// 获取请求地址
	...
	// 获取accessToken
	accessToken := GetBaiduOauthToken(ctx, redisClient, config)
	...
	url := fmt.Sprintf("%srest/2.0/ocr/v1/%s?access_token=%s", config.BaiduOauth.OauthUrl, method, accessToken)
